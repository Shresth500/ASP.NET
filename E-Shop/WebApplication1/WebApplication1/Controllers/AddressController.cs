...
{
    [HttpGet]
    public async Task<IActionResult> GetAddressAsync(PaginationQuerDto pagination)
    {
        var id = User!.FindFirst(ClaimTypes.NameIdentifier)!.Value;
        var data = await repo.findAddressAsync(pagination,int.Parse(id));
    ...
    public  async Task<IActionResult> GetAddressByIdAsync([FromRoute] int id)
    {
        ...
        var data = await repo.findAddressByIdAsync(id, int.Parse(userId));
    ...
    public async Task<IActionResult> AddAddress(AddressDto address)
    {
        ...
        var data = await repo.AddAddressAsync(address, int.Parse(id));
        var addedAddress = mapper.Map<AddressDto>(data);
        return Ok(addedAddress);
    }
    [HttpPut("{AddressId:int}")]
    public async Task<IActionResult> UpdateAddress([FromBody] AddressDto address, [FromRoute] int AddressId)
    {
        var id = User?.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (id == null) return Unauthorized();
        await repo.UpdateAddressAsync(address, AddressId, int.Parse(id));
        return Ok();
    }
    ...
    public async Task<IActionResult> DeleteAddress([FromRoute] int AddressId)
    {
        var id = User?.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (id == null) return Unauthorized();
        await repo.DeleteAddressAsync(AddressId, int.Parse(id));
        return Ok();
    }
}
