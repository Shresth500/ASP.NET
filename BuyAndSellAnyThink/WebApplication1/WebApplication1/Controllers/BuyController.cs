...
{
    [HttpPost]
    public async Task<IActionResult> BuyProduct([FromQuery] BuyProductQueryDto buyProductQueryDto)
    {
        var id = User!.FindFirst(ClaimTypes.NameIdentifier)!.Value;
        var data = await repo.BuyProduct(int.Parse(id), buyProductQueryDto.ProductId, buyProductQueryDto.Quantity);
